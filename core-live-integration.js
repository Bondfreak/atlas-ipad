(()=>{
  ...
  document.querySelectorAll('.topbar .status').forEach(el=>{
    el.textContent=el.textContent.replace(/Atlas v0\.3\.18|Atlas v0\.3\.19/g,`Atlas ${VERSION}`);
  });

  const style=document.createElement('style');
  style.textContent=`
    .coreLive{margin:0 0 12px;padding:11px;border:1px solid #dfe6ec;border-radius:10px;background:#f8fafb;color:#4e5b65}
    .assemblyInfo>.coreLive{margin:8px 12px 10px;max-height:142px;overflow:auto;-webkit-overflow-scrolling:touch}
    ...
    .coreLiveRelations{margin-top:7px;padding-top:7px;border-top:1px solid #e7ecef;font-size:9px;line-height:1.4}.coreLiveRelations b{display:block;margin-bottom:3px;color:#14212b;font-size:9px}
    .coreLiveRelations ul{margin:0;padding-left:14px}.coreLiveRelations li{margin:2px 0}.coreLiveRetry{margin-top:8px;border:0;border-radius:7px;background:#0867ff;color:#fff;padding:6px 9px;font:inherit;font-size:9px;font-weight:700;cursor:pointer}.coreLiveRetry[hidden]{display:none}
  `;
  ...
  function renderRelations(edges){relationsEl.replaceChildren();if(!edges.length){const li=document.createElement('li');li.textContent='Ingen direkte relationer.';relationsEl.appendChild(li);return}for(const edge of edges){const li=document.createElement('li');li.textContent=`${edge.type}: ${edge.from} → ${edge.to}`;relationsEl.appendChild(li)}}
